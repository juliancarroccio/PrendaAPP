...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:root@localhost/dbappscanner'
...
def create_prenda():
    ...
    new_task = Producto(codigoBarra, descripcion_producto, id_color, id_familia, id_industria, id_marca, id_proveedor, id_talle, iva, precio, stock)

    db.session.add(new_task)
    db.session.commit()

    return producto_schema.jsonify(new_task)


@app.route('/prenda', methods=['GET'])
def get_prendas():
    all_tasks = Producto.query.all()
    result = productos_schema.dump(all_tasks)
